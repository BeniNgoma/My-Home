    ...
    <div className="p-8 space-y-7 max-w-[1400px]">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={label} className="card flex items-center gap-4">
            <div className={`p-3 rounded-2xl shrink-0 ${color === 'sage' ? 'bg-sage-100' : color === 'terra' ? 'bg-terra-100' : 'bg-warm-100'}`}>
              <Icon size={22} className={color === 'sage' ? 'text-sage-600' : color === 'terra' ? 'text-terra-500' : 'text-warm-500'} />
            </div>
            <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="card flex items-center gap-4">
          <div className="p-3 bg-sage-100 rounded-2xl shrink-0"><Clock size={22} className="text-sage-600" /></div>
          <div>
        ...
        <div className="card flex items-center gap-4">
          <div className="p-3 bg-terra-100 rounded-2xl shrink-0"><TrendingUp size={22} className="text-terra-500" /></div>
          <div>
        ...
        <div className={`card flex items-center gap-4 ${alertSessions.length > 0 ? 'border-red-200 bg-red-50/60' : ''}`}>
          ...
          <div>
            ...
            <p className={`text-2xl font-bold ${alertSessions.length > 0 ? 'text-red-600' : 'text-warm-900'}`}>{alertSessions.length}</p>
          </div>
        </div>
      </div>
                ...
                {activeSessions.map(s => {
                  ...
                  return (
                    <tr key={s.id} className={isAlert ? 'bg-red-50/60' : 'hover:bg-warm-50/60'}>
                      <td className="table-cell">
                        <div className="flex items-center gap-2.5">
                          <div className="w-7 h-7 rounded-lg bg-sage-100 flex items-center justify-center shrink-0">
                            <span className="text-sage-700 text-[10px] font-bold">{(s as any).agent?.full_name?.[0] ?? '?'}</span>
                          </div>
                          <span className="font-semibold text-warm-900">{(s as any).agent?.full_name}</span>
                        </div>
                      </td>
                      ...
                      <td className="table-cell"><span className="font-bold text-sage-600">{getLiveDuration(s.clock_in_at)}</span></td>
                      <td className="table-cell">
                        <div className="flex items-center gap-2">
                          {isAlert && <span className="text-xs px-2 py-0.5 rounded-full bg-red-100 text-red-700 font-semibold">+12h</span>}
                          {s.gps_alert && (
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-6">
          ...
          <span className="text-xs text-warm-400 bg-warm-50 px-3 py-1 rounded-full border border-warm-200">Last 30 days</span>
        </div>
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={dayStats} margin={{ left: -20, right: 8, top: 4, bottom: 0 }}>
            ...
            <XAxis dataKey="date" tick={{ fontSize: 10, fill: '#9B9286' }} interval={4} axisLine={false} tickLine={false} />
            <YAxis tick={{ fontSize: 10, fill: '#9B9286' }} unit="h" axisLine={false} tickLine={false} />
            <Tooltip
              contentStyle={{ background: 'white', border: '1px solid #E8E5DE', borderRadius: '12px', boxShadow: '0 4px 16px rgba(0,0,0,0.08)', fontSize: 12 }}
